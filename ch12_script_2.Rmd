...
```{r}
model_1 %>%
  augment(newdata = x, ID = "name") %>%
  mutate(.resid = earnings - .fitted) %>%
  slice(which.min(.resid), which.max(.resid))
...
Include either Tier or tier name for public institutions.
...
x %>%
  ...
  tidy(conf.int = TRUE)
```
For each increase in the price of a school the estimated earnings increase by $1,394.
...
We are 95% confident that the true average increase in projected student earnings for those attending a public school lies in the interval calculated.

95% of the time that you perform this exercise, the intervals constructed will contain the true but unknown population parameter
...
```{r}
x %>%
  rep_sample_n(size = nrow(.), replace = TRUE, reps = 100) %>%
  group_by(replicate) %>%
  nest() %>%
  ...
  unnest(tidied) %>%
  group_by(term) %>%
  mutate(conf.low = quantile(estimate, 0.025),
...
```{r}
model_2 %>%
  ggplot(aes(price, earnings, color = as.factor(public))) + geom_point()
```
### Scene 12
